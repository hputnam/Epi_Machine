...
for Sp in "${!Spp[@]}"; do
	...
	echo "$QUERY" "$START" "$STOP" "$SUBJECT"; blastdbcmd -entry "$QUERY" -db db/Spis/Spis -target_only -range "$START"-"$STOP" >> queries/Spis_blastn_query.NOTfasta; 
	sed 's/>/"$SUBJECT"|"$QUERY"|/g' queries/Spis_blastn_query.NOTfasta > blast_res/test_res/Spis_DNMTs_n.fasta
	rm queries/Spis_blastn_query.NOTfasta
done < blast_res/test_res/Spis_DNMTs_blastn.csv
done
...
for Sp in "${!Spp[@]}"; do
	...
	blastp -query "$resDir"/"${Spp[Sp]}"_DNMTs.fasta -db nr -remote \
		-evalue 1e-05 -max_hsps 1 -culling_limit 1 -entrez_query "Scleractinia [Organism]" -out "$resDir"/"${Spp[Sp]}"_DNMTs_nrScler_hits.out
	RID=($(grep RID "$resDir"/"${Spp[Sp]}"_DNMTs_nrScler_hits.out | head -1 | sed -e 's/RID: //g'))
	echo -e "\n Now creating desired output files for blastp search $(date)"
	blast_formatter -rid "$RID" -outfmt "10 qseqid sacc sscinames stitle pident length mismatch gapopen qstart qend sstart send evalue bitscore" -out "$resDir"/"${Spp[Sp]}"_DNMTs_nrScler_hits.csv
