...
```{r}
fasta <- read.fasta(file = "blast_results/final_fasta_files/DNMTs.fasta", clean_name = FALSE) #load fasta file
...
```{r}
stats <- read_csv("blast_results/final_alignment_reports/DNMTs_alignment_report.csv", col_names = c("enzyme", "query", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore"))
...
stat_fasta_clean_top1 <- stat_fasta_clean %>% group_by(spp, enzyme) %>% slice_max(order_by = bitscore, n = 1, with_ties = FALSE); unique(stat_fasta_clean_b)
```
